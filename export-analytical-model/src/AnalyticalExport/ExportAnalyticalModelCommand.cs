...
public sealed class ExportAnalyticalModelCommand : IExternalCommand
{
    ...
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        ...
        if (document.IsFamilyDocument)
        {
            Autodesk.Revit.UI.TaskDialog.Show(
                "Analytical Export",
                "Open a Revit project document before running the analytical export.");
        ...
        try
        {
            ExportResult export = BuildExport(document);
            ...
            File.WriteAllText(path, JsonSerializer.Serialize(export.Model, JsonOptions));

            string summary =
                $"Wrote analytical export JSON to:{Environment.NewLine}{path}{Environment.NewLine}{Environment.NewLine}" +
                $"Nodes: {export.Model.Nodes.Count}{Environment.NewLine}" +
                $"Lines: {export.Model.Lines.Count}{Environment.NewLine}" +
                $"Areas: {export.Model.Areas.Count}";

            if (export.Warnings.Count > 0)
            {
                IEnumerable<string> previewWarnings = export.Warnings.Take(10);
                summary +=
                    $"{Environment.NewLine}{Environment.NewLine}Warnings: {export.Warnings.Count}{Environment.NewLine}- " +
                    string.Join(Environment.NewLine + "- ", previewWarnings);

                if (export.Warnings.Count > 10)
                {
                    summary += $"{Environment.NewLine}- ...";
                }
            }

            Autodesk.Revit.UI.TaskDialog.Show("Analytical Export", summary);
    ...
    private static string? PromptForJsonPath()
    {
        ...
        dialog.Title = "Export analytical model JSON";
        ...
        {
            dialog.FileName = "analytical_export.json";
        }

        return dialog.ShowDialog() == WinForms.DialogResult.OK ? dialog.FileName : null;
    }

    private static ExportResult BuildExport(Document document)
    {
        ModelDto model = new();
        ...
        return new ExportResult(model, warnings);
    }
    ...
    private sealed record ExportResult(ModelDto Model, List<string> Warnings);
}
