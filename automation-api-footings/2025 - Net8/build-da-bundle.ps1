...
$projectPath = Join-Path $automationRoot "src/PadFoundationsDA/PadFoundationsDA.csproj"
$bundleRoot = Join-Path $automationRoot "PadFoundationsDA.bundle"
$bundleContentsPath = Join-Path $bundleRoot "Contents"
$bundleManifestPath = Join-Path $bundleRoot "PackageContents.xml"
$addinManifestPath = Join-Path $bundleContentsPath "PadFoundationsDA8.addin"
$filesRoot = Join-Path $automationRoot "files"
$zipPath = Join-Path $filesRoot "PadFoundationsDA.bundle.zip"
...
$buildOutputPath = Join-Path $automationRoot "src/PadFoundationsDA/bin/$Configuration/$targetFramework"
...
Get-ChildItem -Path $bundleContentsPath -File |
    Where-Object { $_.Name -ne "PadFoundationsDA8.addin" } |
    Remove-Item -Force
