...
public sealed class CreatePileFoundationsCommand : IExternalCommand
{
    ...
    private static IReadOnlyList<PileFoundationRequest> ReadJson(string path)
    {
        ...
        foreach (JsonElement item in array.EnumerateArray())
        {
            ...
            PileFoundationTypeParameters parameters = ReadParameters(item, index);
            string typeName = ReadOptionalString(item, "typeName") ?? DefaultTypeName;
            ...
            {
                FamilyName = ReadOptionalString(item, "familyName") ?? DefaultFamilyName,
                TypeName = typeName,
                TargetTypeName = targetTypeName,
                Units = ReadOptionalString(item, "units") ?? DefaultUnits,
    ...
    private static PileFoundationTypeParameters ReadParameters(JsonElement item, int index)
    {
        if (!TryGetProperty(item, "parameters", out JsonElement parametersElement))
        {
        ...
        {
            throw new InvalidOperationException($"Pile foundation item {index} property 'parameters' must be a JSON object.");
        }
    ...
    private static string ReadRequiredString(JsonElement item, int index, string name)
    {
        string? value = ReadOptionalString(item, name);
        if (string.IsNullOrWhiteSpace(value))
        {
            throw new InvalidOperationException(
                $"Pile foundation item {index} is missing required string property '{name}'.");
        }
