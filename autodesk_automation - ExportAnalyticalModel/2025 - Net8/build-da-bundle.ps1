...
$projectPath = Join-Path $automationRoot "src/AnalyticalExportDA/AnalyticalExportDA.csproj"
$bundleRoot = Join-Path $automationRoot "AnalyticalExportDA.bundle"
$bundleContentsPath = Join-Path $bundleRoot "Contents"
$bundleManifestPath = Join-Path $bundleRoot "PackageContents.xml"
$addinManifestPath = Join-Path $bundleContentsPath "AnalyticalExportDA8.addin"
$filesRoot = Join-Path $automationRoot "files"
$zipPath = Join-Path $filesRoot "AnalyticalExportDA.bundle.zip"
...
$buildOutputPath = Join-Path $automationRoot "src/AnalyticalExportDA/bin/$Configuration/$targetFramework"
...
Get-ChildItem -Path $bundleContentsPath -File |
    Where-Object { $_.Name -ne "AnalyticalExportDA8.addin" } |
    Remove-Item -Force
